    ...
    while True:
        ...
        for column_index in range(COLUMN_COUNT):
            for row_index in range(ROW_COUNT):
                ...
                if not tile['open']:
                    pygame.draw.rect(screen,GRAY,pygame.Rect(column_index * CELL_SIZE,row_index * CELL_SIZE,CELL_SIZE,CELL_SIZE),1)

        if game_over > 0 :
            if game_over ==SUCCESS:
                success_imgae = large_font.render('Success',True,RED)
                screen.blit(success_imgae,success_imgae.get_rect(centerx=SCREEN_WIDTH//2,centery=SCREEN_HEIGHT//2))
