...
x = MaxPool2D((2, 2), padding='same')(x)                                       #14*14
...
noise_factor = 0.5
conv_x_train_noise = conv_x_train + np.random.normal(loc=0.0, scale=1.0, size=conv_x_train.shape) * noise_factor
conv_x_train_noise = np.clip(conv_x_train_noise, 0.0, 1.0)
noise_factor = 0.5
conv_x_test_noise = conv_x_test + np.random.normal(loc=0.0, scale=1.0, size=conv_x_test.shape) * noise_factor
conv_x_test_noise = np.clip(conv_x_test_noise, 0.0, 1.0)

fit_hist = autoencoder.fit(conv_x_train_noise, conv_x_train, epochs=100, batch_size=256, validation_data=(conv_x_test_noise, conv_x_test))
